...
-- Step 1: Make user_id nullable to allow public/system personas
ALTER TABLE personas ALTER COLUMN user_id DROP NOT NULL;

-- Step 2: Add is_public column if it doesn't exist
...
ALTER TABLE personas ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();

-- Step 4: Insert celebrity personas with NULL user_id (making them public)
...
-- Step 5: Create index for faster queries on public personas
CREATE INDEX IF NOT EXISTS idx_personas_is_public ON personas(is_public) WHERE is_public = true;

-- Step 6: Update RLS (Row Level Security) policy to allow anyone to read public personas
...
-- Create a more permissive policy for reading personas
