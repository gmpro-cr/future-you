...
-- First, create a system user if it doesn't exist (for public personas)
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, updated_at)
VALUES (
  '00000000-0000-0000-0000-000000000000',
  'system@future-you.app',
  crypt('system-user-no-login', gen_salt('bf')),
  now(),
  now(),
  now()
)
...
-- Add column to personas table if it doesn't exist (is_public)
ALTER TABLE personas ADD COLUMN IF NOT EXISTS is_public BOOLEAN DEFAULT false;

-- Create index for faster queries on public personas
CREATE INDEX IF NOT EXISTS idx_personas_is_public ON personas(is_public) WHERE is_public = true;

-- Update RLS (Row Level Security) policy to allow anyone to read public personas
